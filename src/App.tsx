...
export default function App() {
  ...
  const generarProyeccion = async () => {
    ...
    const generarProyeccionConIA = async () => {
    if (selectedDays.length === 0) return alert("Selecciona al menos un día.");
    const nvidiaKey = import.meta.env.VITE_NVIDIA_API_KEY;
    if (!nvidiaKey) return alert("Falta la API Key de NVIDIA en el archivo .env.local");

    setIsLoading(true);
    setStatusMsg("Extrayendo historial para la IA...");

    // 1. Obtener historial (mismo método que el anterior)
    const startDate = new Date(selectedYear, selectedMonth - 2, 1);
    const endDate = new Date(selectedYear, selectedMonth, 1);
    ...
    while (true) {
      const { data, error } = await supabase.from('historial_viajes')
        .select('unidad, cod_turno, is_tm, tipo_dia')
        .gte('fecha_salida', startDate.toISOString())
        .lt('fecha_salida', endDate.toISOString())
        .range(from, from + step - 1);
      if (error) break;
    ...
    if (hist.length === 0) {
      alert("No hay historial suficiente.");
      setIsLoading(false);
      return;
    }
    ...
    // 2. Comprimir el historial para que la IA lo entienda fácil (Ahorro de tokens)
    const resumenUnidades: Record<string, any> = {};
    hist.forEach(t => {
      const franja = t.is_tm ? 'TM' : 'TT';
      if (!resumenUnidades[t.unidad]) resumenUnidades[t.unidad] = {};
      const key = `${franja}_${t.tipo_dia}`; // Ej: TM_Semana, TT_Sabado
      if (!resumenUnidades[t.unidad][key]) resumenUnidades[t.unidad][key] =[];
      resumenUnidades[t.unidad][key].push(t.cod_turno);
    });

    const prompt = `
    Eres un experto planificador logístico de transporte.
    Te daré un historial de viajes de varias unidades en formato JSON. Las llaves son "Franja_TipoDia" (Ej: TM_Semana es Turno Mañana en días de semana).
    Tu tarea es deducir el turno más lógico a asignar para el próximo mes para cada unidad, franja y tipo de día.
    
    REGLA ESTRICTA: Devuelve ÚNICAMENTE un JSON válido, sin texto adicional, sin formato markdown (\`\`\`), con esta estructura exacta:
    {
      "L540-001": {
        "TM_Semana": "5401",
        "TT_Semana": "5402",
    ...
    Si una unidad no tiene un patrón claro o no operó en esa franja, omítela o devuelve null.

    HISTORIAL A ANALIZAR:
    ${JSON.stringify(resumenUnidades)}
    `;

    try {
      // Ahora llamamos a nuestro propio backend en Vercel
      ...
      console.error(error); 
      alert("Error en Mistral AI o en el servidor."); 
      setIsLoading(false);
    }
  };
